...
# importiert JSON-Datenbank von Vivien Gräf in Postgres
...
try:
    ...
    with open("/home/codetree/baum-backend/database/Tablefiles/json/db.json", "r") as f:
...
sql_create_table_geolocation = "CREATE TABLE IF NOT EXISTS geolocation(ID serial PRIMARY KEY, latitude numeric NOT NULL, longitude numeric NOT NULL)"
sql_create_table_object = """CREATE TABLE IF NOT EXISTS object(
...
sql_create_table_located =  "CREATE TABLE IF NOT EXISTS located(object_id integer NOT NULL REFERENCES object(ID), sensor_id integer NOT NULL REFERENCES sensor(ID))"
try:
    cur.execute(sql_create_table_geolocation)
    cur.execute("CREATE TABLE IF NOT EXISTS name(ID serial PRIMARY KEY, german text NOT NULL, botanical text NOT NULL)")
    cur.execute("CREATE TABLE IF NOT EXISTS description(ID serial PRIMARY KEY, name_id integer NOT NULL REFERENCES name(id), origin text NOT NULL, leaf_shape text NOT NULL, general_description text NOT NULL)")
    cur.execute("CREATE TABLE IF NOT EXISTS flowering(month smallint NOT NULL, description_id integer NOT NULL REFERENCES description(ID))")
    cur.execute("CREATE TABLE IF NOT EXISTS location(ID serial PRIMARY KEY, geolocation_id integer NOT NULL REFERENCES geolocation(id), height integer NOT NULL)")
    cur.execute("CREATE TABLE IF NOT EXISTS sensor(ID serial PRIMARY KEY, measured_variable text NOT NULL, installation_date date NOT NULL, location_id integer NOT NULL REFERENCES location(id))")
    
    cur.execute(sql_create_table_object)
    cur.execute(sql_create_table_located)
    cur.execute("CREATE TABLE IF NOT EXISTS measured_values(ID serial PRIMARY KEY, value integer NOT NULL, timestamp timestamp NOT NULL, sensor_ID integer NOT NULL REFERENCES sensor(id))")    
except psycopg2.Error as e:
    print(e)
description_ids = {}    
