...
// Setzt für alle Objekte ein Marker in der Karte
	makeMaker(map: L.Map, path: string) {
          ...
          console.log(t.ID)
            ...
            if (r.object_id === t.ID) {
              ...
              console.log(t.ID)
							...
							if (date.getFullYear() === now.getFullYear() && date.getMonth() === now.getMonth() && date.getDate() === now.getDate() && date > now) {
								edge = 'red';
								break;
							}
						}
         ...
         */
              console.log(t.ID)
					this.api.getData('latest_measured_value/' + t.ID).subscribe(measurement => {
